...
import com.valensas.undetected.chrome.driver.util.PatcherUtil;
import com.valensas.undetected.chrome.driver.util.SysUtil;
...
public class ChromeDriverBuilder {
  private final Logger logger = LoggerFactory.getLogger(ChromeDriverBuilder.class);
  ...
  private void buildPatcher(String driverExecutablePath) throws RuntimeException {
    // patcher
    PatcherUtil patcher = new PatcherUtil(driverExecutablePath);
  ...
  private void setHostAndPort(ChromeOptions chromeOptions) throws RuntimeException {
    // debug host and port
    ...
    try {
      Field experimentalOptions =
          chromeOptions.getClass().getSuperclass().getDeclaredField("experimentalOptions");
      ...
      if (experiments != null && experiments.get("debuggerAddress") != null) {
        return;
      }
    } catch (Exception ignored) {
    }
  ...
  private void setUserDataDir(ChromeOptions chromeOptions) throws RuntimeException {
    ...
    if (_userDataDir == null || _userDataDir.isEmpty()) {
  ...
  private void setLanguage(ChromeOptions chromeOptions) {
    if (args != null) {
      for (String arg : args) {
  ...
  private void setBinaryLocation(ChromeOptions chromeOptions, String binaryLocation) {
  ...
  /** step8, set log level */
  private void setLogLevel(ChromeOptions chromeOptions) {
    if (args != null) {
      for (String arg : args) {
  ...
  private void handlePrefs(String userDataDir, Map<String, Object> prefs) throws RuntimeException {
    ...
    if (new File(prefsFile).exists()) {
      try (BufferedReader br =
          new BufferedReader(new FileReader(prefsFile, StandardCharsets.ISO_8859_1))) {
        String line;
    ...
    Thread outputThread =
        new Thread(
            () -> {
              try {
                BufferedReader br =
                    new BufferedReader(new InputStreamReader(browser.getInputStream()));
                String buff;
                while ((buff = br.readLine()) != null) {
                  System.out.println(buff);
                }
              } catch (Exception e) {
                logger.error("chrome open fail", e);
              }
            });
    ...
    Thread errorPutThread =
        new Thread(
            () -> {
              try {
                BufferedReader er =
                    new BufferedReader(new InputStreamReader(browser.getErrorStream()));
                String errors;
   ...
   *
   * @param options chromeOptions required no prefs args in experiment map, if has prefs, use param
   *     prefs
  ...
  public ChromeDriver build(
    ...
    setHostAndPort(chromeOptions);

    // step3, set user data dir
    setUserDataDir(chromeOptions);

    // step4, set language
    setLanguage(chromeOptions);

    // step5, set binaryLocation
    setBinaryLocation(chromeOptions, binaryLocation);
  ...
  public ChromeDriver build(
      ...
      boolean needPrintChromeInfo) {
    // operator headless
    boolean headless = isHeadless(options);
  ...
  private static boolean isHeadless(ChromeOptions options) {
  ...
  public ChromeDriver build(ChromeOptions options, String driverExecutablePath) {
    return this.build(
        options, driverExecutablePath == null ? "" : driverExecutablePath, true, false);
  }
  ...
  private int findFreePort() {
    try (ServerSocket socket = new ServerSocket(0)) {
      ...
      logger.error("can't find free port", e);
      return -1;
    }
  }
